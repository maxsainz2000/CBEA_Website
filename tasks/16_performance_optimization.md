...
## Acceptance Criteria
- [x] ~~`app/page.tsx` uses `export const revalidate = 60` instead of `export const dynamic = 'force-dynamic'`.~~ (Superseded by Task 19)
...
- [x] ~~The homepage loads from CDN cache on repeat visits (verify via response headers or build output).~~ (Superseded by Task 19)
